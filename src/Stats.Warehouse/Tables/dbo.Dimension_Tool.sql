...
(
    ...
    CONSTRAINT [PK_Dimension_Tool] PRIMARY KEY CLUSTERED ([Id] ASC) WITH (STATISTICS_NORECOMPUTE = ON)
);
GO

CREATE UNIQUE NONCLUSTERED INDEX [Dimension_Tool_NCI_ToolId_ToolVersion_FileName]
    ON [dbo].[Dimension_Tool]([ToolId] ASC, [ToolVersion] ASC, [FileName] ASC) WITH (STATISTICS_NORECOMPUTE = ON);
GO

CREATE NONCLUSTERED INDEX [Dimension_Tool_Lowercased]
	ON [dbo].[Dimension_Tool]([LowercasedToolId] ASC, [LowercasedToolVersion] ASC, [LowercasedFileName] ASC) INCLUDE ([Id], [ToolId], [ToolVersion], [FileName]) WITH (STATISTICS_NORECOMPUTE = ON);
GO