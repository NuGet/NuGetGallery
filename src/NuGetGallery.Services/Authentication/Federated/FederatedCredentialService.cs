...
namespace NuGetGallery.Services.Authentication
{
    ...
    public class FederatedCredentialService : IFederatedCredentialService
    {
        ...
        public async Task<AddFederatedCredentialPolicyResult> AddEntraIdServicePrincipalPolicyAsync(User user, User packageOwner, EntraIdServicePrincipalCriteria criteria)
        {
            if (user is Organization)
            {
                return AddFederatedCredentialPolicyResult.BadRequest(
                    $"Policy user '{user.Username}' is an organization. Creating federated credential trust policies for organizations is not supported.");
            }

            var testScope = new Scope(packageOwner, NuGetPackagePattern.AllInclusivePattern, NuGetScopes.All);
            if (!_credentialBuilder.VerifyScopes(user, [testScope]))
            {
                return AddFederatedCredentialPolicyResult.BadRequest(
        ...
        public async Task DeletePolicyAsync(FederatedCredentialPolicy policy)
        {
            ...
            {
                await _authenticationService.RemoveCredential(policy.User, credential, commitChanges: false);
            }
        ...
        public async Task<GenerateApiKeyResult> GenerateApiKeyAsync(string username, string bearerToken)
        {
            ...
            var policies = _repository.GetPoliciesForUser(currentUser.Key);
            ...
            var packageOwner = _userService.FindByKey(policyEvaluation.Policy.OwnerKey);
            policyEvaluation.Policy.Owner = packageOwner;
        ...
        {
            evaluation.Policy.LastMatched = _dateTimeProvider.UtcNow;
