...
namespace NuGetGallery.Services.Helpers
{
    public static class StreamHelper
    {
        ...
        public static TruncatedStream GetTruncatedStreamWithMaxSize(this Stream stream, int maxSize)
        {
            if (stream == null)
            {
            ...
            try
            {
                ...
                while ((bytesRead = stream.Read(buffer, 0, GetNeededBytesToRead(totalBytesRead, maxSize))) > 0)
                {
