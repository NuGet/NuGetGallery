...
namespace GitHubAdvisoryTransformer.Collector
{
    public class AdvisoryQueryService : IAdvisoryQueryService
    {
        ...
        {
            Console.WriteLine($"Fetching advisories updated since {lastUpdated}");
            ...
            while (lastAdvisoryEdges.Any())
            {
                ...
                Console.WriteLine($"Fetching advisories with cursor after {lastCursor}");
        ...
        private async Task<SecurityAdvisory> FetchAllVulnerabilitiesAsync(SecurityAdvisory advisory, CancellationToken token)
        {
            ...
            {
                Console.WriteLine($"Fetching more vulnerabilities for advisory with database key {advisory.DatabaseId} / GHSA ID {advisory.GhsaId}", advisory.DatabaseId, advisory.GhsaId);
