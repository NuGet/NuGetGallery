...
namespace Stats.AzureCdnLogs.Common
{
    public static class ExtensionsUtils
    {
        ...
        public static string[] GetSegmentsFromCSV(string line)
        {
            ...
            {
                if (!segments[i].Contains("\"") || (segments[i].StartsWith("\"", StringComparison.Ordinal) && segments[i].EndsWith("\"", StringComparison.Ordinal)))
                {
                ...
                else
                {
                    ...
                    while (i < segments.Length && !segments[i].EndsWith("\"", StringComparison.Ordinal))
                    {
