...
namespace Stats.AzureCdnLogs.Common
{
    public class CdnLogCustomFieldParser
    {
        public static IDictionary<string, string> Parse(string customField)
        {
            ...
            var customFields = Regex.Matches(temp, @"(?<key>[^\s]+[:]{1})[\s]{1}(?<value>([-]{1}\s)|(\w+((\s)|([-]{1}\w+\s)))|(([\{]{1}[0-9-a-fA-F]{8}[-]{1}[0-9a-fA-F]{4}[-]{1}[0-9a-fA-F]{4}[-]{1}[0-9a-fA-F]{4}[-]{1}[0-9a-fA-F]{12}[\}]{1}[;]?)+\s)|([a-zA-Z0-9\/\.\s\(\;\)]*))");
