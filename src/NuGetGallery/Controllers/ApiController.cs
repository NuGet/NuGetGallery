...
namespace NuGetGallery
{
        ...
        : AppController
    {
        ...
        private async Task<HttpStatusCodeWithBodyResult> VerifyPackageKeyInternalAsync(User user, Credential credential, string id, string version)
        {
            ...
            if (!PackagePermissionsService.HasPermission(package, user, PackagePermissionsService.Permission.Upload))
            {
                ...
                try
                {
                    ...
                    using (var packageToPush = new PackageArchiveReader(packageStream, leaveStreamOpen: false))
                    {
                        ...
                        else
                        {
                            ...
                            if (!PackagePermissionsService.HasPermission(packageRegistration, user, PackagePermissionsService.Permission.Upload))
                            {
