...
namespace NuGetGallery.Controllers
{
	...
	public class AdminApiController : AppController
	{
		...
		{
			AdminReflowRequest request;
			try
			{
				Request.InputStream.Position = 0;
				using (var reader = new StreamReader (Request.InputStream, System.Text.Encoding.UTF8, true, 4096, leaveOpen: true))
				{
					var body = reader.ReadToEnd();
					request = JsonConvert.DeserializeObject<AdminReflowRequest> (body);
				}
			}
			catch (JsonException)
			{
			...
			{
				return Json (HttpStatusCode.BadRequest,
					new { error = "The 'packages' field is required and must contain at least one entry." });
			}

			if (request.Packages.Count > MaxPackageCount)
			{
				return Json (HttpStatusCode.BadRequest,
					new { error = $"The 'packages' field must contain at most {MaxPackageCount} entries." });
			}
			...
			foreach (var entry in request.Packages)
			{
				if (string.IsNullOrWhiteSpace (entry?.Id) || string.IsNullOrWhiteSpace (entry?.Version))
				{
					results.Add (new AdminReflowPackageResult
					{
						...
						Status = AdminReflowStatus.Invalid
					});
					continue;
				}
				...
				if (package == null || package.PackageStatusKey == PackageStatus.Deleted)
				{
					results.Add (new AdminReflowPackageResult
					{
						...
						Status = AdminReflowStatus.NotFound
					});
					continue;
				}
				...
				results.Add (new AdminReflowPackageResult
				{
					Id = entry.Id,
					Version = normalizedVersion,
					Status = AdminReflowStatus.Accepted
				});
			...
			{
				return Json (HttpStatusCode.BadRequest,
					new AdminReflowResponse { Results = results });
			}
