            ...
            return packages.Select(p => new V2FeedPackage
                {
                    Id = p.PackageRegistration.Id,
                    Version = p.Version,
                    NormalizedVersion = p.GetNormalizedVersion(),
                    Authors = p.FlattenedAuthors,
                    Copyright = p.Copyright,
                    Created = p.Created,
                    Dependencies = p.FlattenedDependencies,
                    Description = p.Description,
                    DownloadCount = p.PackageRegistration.DownloadCount,
                    GalleryDetailsUrl = siteRoot + "packages/" + p.PackageRegistration.Id + "/" + p.GetNormalizedVersion(),
                    IconUrl = p.IconUrl,
                    IsLatestVersion = p.IsLatestStable,
                    ...
                    ProjectUrl = p.ProjectUrl,
                    ReleaseNotes = p.ReleaseNotes,
                    ReportAbuseUrl = siteRoot + "package/ReportAbuse/" + p.PackageRegistration.Id + "/" + p.GetNormalizedVersion(),
                    RequireLicenseAcceptance = p.RequiresLicenseAcceptance,
                    Published = p.Listed ? p.Published : UnpublishedDate,
                    Summary = p.Summary,
                    Tags = p.Tags,
