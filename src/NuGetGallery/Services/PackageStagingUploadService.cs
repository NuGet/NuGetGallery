    ...
    public class PackageStagingUploadService : IPackageStagingUploadService
    {
            ...
            bool? listed = null)
        {
            ...
            try
            {
                ...
                return await ProcessUploadAsync(currentUser, httpContext, upload, target, group, listed);
            }
            ...
            Stream packageFile)
        {
            ...
            try
            {
                ...
                return await ProcessUploadAsync(currentUser, httpContext, upload, target, requestedGroup: null, listed: null);
            }
            catch (Exception exception) when (IsInvalidPackage(exception))
            {
                exception.Log();
        ...
        private PackageStagingResult ResolveUiReplacementTarget(
            ...
            out StagingTarget target)
        {
            var hasMatchingId = string.Equals(authorizedStagedPackage.StagedPackageIdentity.Package.PackageRegistration.Id, upload.Id, StringComparison.OrdinalIgnoreCase);
            var hasMatchingVersion = string.Equals(authorizedStagedPackage.StagedPackageIdentity.Package.NormalizedVersion, upload.NormalizedVersion, StringComparison.OrdinalIgnoreCase);
            ...
            return ResolveTarget(
                ...
                existingPackage,
                currentAttempt,
                authorizedStagedPackage.StagedPackageIdentity.Owner,
                allowCreate: false,
                out target);
        }
            ...
            out StagingTarget target)
        {
            ...
            if (currentAttempt.StagedPackageIdentity.OwnerKey != owner.Key)
            {
                return CreateExistingPackageConflict(upload.Id, upload.PackageMetadata.Version);
            }
            ...
            bool? listed)
        {
            ...
            if ((isValidating || isReady) && isIdentical)
            {
                ...
                if ((requestedGroup != null && identity.StagingGroupKey != requestedGroup.Key) || (listed.HasValue && identity.Package.Listed != listed.Value))
                {
                    try
                    {
                    ...
                    {
                        exception.Log();
                        return PackageStagingResult.Error(HttpStatusCode.Conflict, "The staging group or package changed during upload. Retry the upload.");
                    }
                }
            ...
            upload.PackageFile.Position = 0;
            var creatingGroup = requestedGroup?.Key == 0;
