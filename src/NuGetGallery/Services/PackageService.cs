...
#pragma warning restore 618
        ...
        private static void ValidateNuGetPackageMetadata(PackageMetadata packageMetadata)
        {
            ...
            if (packageMetadata.Id.Length > PackageRegistration.MaxPackageIdLength)
            {
                throw new EntityException(Strings.NuGetPackagePropertyTooLong, "Id", PackageRegistration.MaxPackageIdLength);
            }
            ...
            if (packageMetadata.GetDependencyGroups() != null)
            {
                ...
                foreach (var dependency in packageDependencies.SelectMany(s => s.Packages))
                {
                    ...
                    if (dependency.Id != null && dependency.Id.Length > PackageRegistration.MaxPackageIdLength)
                    {
                        throw new EntityException(Strings.NuGetPackagePropertyTooLong, "Dependency.Id", PackageRegistration.MaxPackageIdLength);
                    }
