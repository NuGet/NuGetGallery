...
namespace NuGetGallery.Infrastructure.Mail
{
    ...
    {
        public static string ParsePlainText(this ValidationIssue validationIssue, string announcementsUrl, string twitterUrl)
        {
        ...
        public static string ParseMarkdown(this ValidationIssue validationIssue, string announcementsUrl, string twitterUrl)
        {
            switch (validationIssue.IssueCode)
            {
                case ValidationIssueCode.PackageIsSigned:
        ...
        public static string ParseHtml(this ValidationIssue validationIssue, string announcementsUrl, string twitterUrl)
        {
            switch (validationIssue.IssueCode)
            {
                case ValidationIssueCode.PackageIsSigned:
                    return $@"This package could not be published since it is signed. We do not accept signed packages at this moment. To be notified about package signing and more, watch our <a href=""{announcementsUrl}"">Announcements</a> page or follow us on <a href=""{twitterUrl}"">Twitter</a>.";
                case ValidationIssueCode.ClientSigningVerificationFailure:
                    var clientIssue = (ClientSigningVerificationFailure)validationIssue;
                    return clientIssue != null
                        ? $"<b>{clientIssue.ClientCode}</b>: {clientIssue.ClientMessage}"
                        : "This package's signature was unable to be verified.";
                case ValidationIssueCode.PackageIsZip64:
                    return "Zip64 packages are not supported.";
                case ValidationIssueCode.OnlyAuthorSignaturesSupported:
                ...
                case ValidationIssueCode.AuthorCounterSignaturesNotSupported:
                    return "Author countersignatures are not supported.";
                case ValidationIssueCode.PackageIsNotSigned:
                    return @"This package must be signed with a registered certificate. <a href=""https://aka.ms/nuget-signed-ref"">Read more...</a>";
                case ValidationIssueCode.PackageIsSignedWithUnauthorizedCertificate:
                    var certIssue = (UnauthorizedCertificateFailure)validationIssue;
                    return $@"The package was signed, but the signing certificate {(certIssue != null ? $"(SHA-1 thumbprint {certIssue.Sha1Thumbprint})" : string.Empty)} is not associated with your account. You must register this certificate to publish signed packages. <a href=""https://aka.ms/nuget-signed-ref"">Read more...</a>";
                case ValidationIssueCode.SymbolErrorCode_ChecksumDoesNotMatch:
                    return "The checksum does not match for the dll(s) and corresponding pdb(s).";
                case ValidationIssueCode.SymbolErrorCode_MatchingPortablePDBNotFound:
                    return "The uploaded symbols package contains pdb(s) for a corresponding dll(s) not found in the nuget package.";
