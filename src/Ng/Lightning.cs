...
namespace Ng
{
    class Lightning
    {
        ...
        private static void PrintUsage()
        {
            ...
            Console.WriteLine("  ng lightning -command prepare -outputFolder <output-folder> -catalogIndex <catalog-index-url> -templateFile <template-file> -batchSize 2000 -storageAccount <connection-string> -container <container> -baseAddress <base-address> [-verbose true|false]");
            Console.WriteLine();
            ...
            Console.WriteLine("  -baseAddress <base-address>");
            ...
            Console.WriteLine("  ng lightning -command strike -indexFile <index-file> -cursorFile <cursor-file> -storageAccount <connection-string> -container <container> -baseAddress <base-address> [-verbose true|false]");
            Console.WriteLine();
        ...
        private static async Task PrepareAsync(TextWriter log, IDictionary<string, string> arguments)
        {
            ...
            string container = CommandHelpers.Get(arguments, "container");
            string baseAddress = CommandHelpers.Get(arguments, "baseAddress");
            ...
            var storageFactory = (StorageFactory)new AzureStorageFactory(account, container);
                ...
                using (var cursorCommandStreamWriter = new StreamWriter(Path.Combine(outputFolder, cursorCommandFileName)))
                {
                    using (var cursorTextStreamWriter = new StreamWriter(Path.Combine(outputFolder, cursorTextFileName)))
                    {
                        var commandStreamContents = templateFileContents
                            ...
                            .Replace("[storage]", storageAccount)
                            .Replace("[container]", container)
                            .Replace("[baseaddress]", baseAddress);
        ...
        private static async Task StrikeAsync(TextWriter log, IDictionary<string, string> arguments)
        {
            ...
            var collectorHttpClient = new CollectorHttpClient();
            var account = CloudStorageAccount.Parse(storageAccount);
            var storageFactory = (StorageFactory)new AzureStorageFactory(account, container, null, new Uri(baseAddress));
                ...
                while (!string.IsNullOrEmpty(line) &&  !line.Contains(endElement) && !indexStreamReader.EndOfStream)
                {
                    ...
                    try
                    {
                        ...
                        while (!string.IsNullOrEmpty(line) && !line.Contains("Element@") && !indexStreamReader.EndOfStream)
                        {
                            ...
                            {
                                // Process graphs
                                await ProcessGraphsAsync(packageId, sortedGraphs, storageFactory, baseAddress);
