...
namespace NuGetGallery
{
    public class StatisticsControllerFacts : TestContainer
    {
        ...
        public async Task StatisticsHomePage_ValidateReportStructureAndAvailability()
        {
            ...
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("recentpopularity.json")).Returns(Task.FromResult(new ReportBlob(fakePackageReport, DateTime.MinValue)));
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("recentpopularitydetail.json")).Returns(Task.FromResult(new ReportBlob(fakePackageVersionReport, null)));
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("recentcommunitypopularity.json")).Returns(Task.FromResult(new ReportBlob(fakePackageReport, DateTime.MinValue)));
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("recentcommunitypopularitydetail.json")).Returns(Task.FromResult(new ReportBlob(fakePackageVersionReport, null)));

            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("nugetclientversion.json")).Returns(Task.FromResult(new ReportBlob(fakeNuGetClientVersion, DateTime.MinValue)));
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("last6weeks.json")).Returns(Task.FromResult(new ReportBlob(fakeLast6Weeks, updatedUtc)));
        ...
        public async Task StatisticsHomePage_ValidateFullReportStructureAndAvailability()
        {
            ...
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("recentpopularity.json")).Returns(Task.FromResult(new ReportBlob(fakePackageReport, DateTime.UtcNow)));
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("recentpopularitydetail.json")).Returns(Task.FromResult(new ReportBlob(fakePackageVersionReport, DateTime.UtcNow)));
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("nugetclientversion.json")).Returns(Task.FromResult(new ReportBlob(fakeNuGetClientVersion, DateTime.UtcNow)));
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("last6weeks.json")).Returns(Task.FromResult(new ReportBlob(fakeLast6Weeks, DateTime.UtcNow)));
        ...
        public async Task StatisticsHomePage_PackageVersions_ValidateReportStructureAndAvailability()
        {
            ...
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("recentpopularitydetail.json")).Returns(Task.FromResult(new ReportBlob(fakePackageVersionReport, updatedUtc1)));
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load("recentcommunitypopularitydetail.json")).Returns(Task.FromResult(new ReportBlob(fakePackageVersionReport, updatedUtc2)));
        ...
        public async Task StatisticsHomePage_Per_Package_ValidateReportStructureAndAvailability()
        {
            ...
            var updatedUtc = new DateTime(2001, 01, 01, 10, 20, 30);
            fakeReportService.Setup(x => x.GetContainer(ContainerName).Load(reportName)).Returns(Task.FromResult(new ReportBlob(fakeReport, updatedUtc)));
