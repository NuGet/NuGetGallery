	...
	public class AdminApiControllerFacts
	{
		public class TheReflowMethod
		{
			...
			public void Returns400WhenBodyIsInvalidJson ()
			{
				...
				var result = controller.Reflow() as JsonResult;
			...
			{
				var request = new AdminReflowRequest { Packages = null };
			...
			{
				var request = new AdminReflowRequest { Packages = new List<AdminReflowPackageIdentity>() };
				var controller = CreateController (request: request);
			...
			public void Returns400WhenPackagesExceeds100 ()
			{
				...
				var request = new AdminReflowRequest { Packages = packages };
				var controller = CreateController (request: request);
			...
			{
				var request = new AdminReflowRequest
				{
				...
				var response = GetResponseData<AdminReflowResponse> (result);
				Assert.Single (response.Results);
				Assert.Equal (AdminReflowStatus.NotFound, response.Results[0].Status);
			}
			...
			public void Returns202WithAcceptedPackages ()
			{
				...
				var response = GetResponseData<AdminReflowResponse> (result);
				Assert.Single (response.Results);
				Assert.Equal (AdminReflowStatus.Accepted, response.Results[0].Status);

				mockTelemetry.Verify (
					t => t.TrackAdminApiReflow (1, 1, "test reflow", It.IsAny<string>()),
					Times.Once);
			}
			...
			public void Returns202WithMixedStatuses ()
			{
				...
				Assert.Equal (AdminReflowStatus.Accepted, response.Results[0].Status);
				Assert.Equal (AdminReflowStatus.NotFound, response.Results[1].Status);
				Assert.Equal (AdminReflowStatus.NotFound, response.Results[2].Status);
				Assert.Equal (AdminReflowStatus.Accepted, response.Results[3].Status);
			}
			...
			public void ReturnsInvalidForBadVersionString ()
			{
				...
				Assert.Equal (AdminReflowStatus.Invalid, response.Results[0].Status);
				Assert.Equal (AdminReflowStatus.Accepted, response.Results[1].Status);
			}
			...
			public void ReturnsInvalidForNullIdOrVersion ()
			{
				...
				Assert.Equal (AdminReflowStatus.Invalid, response.Results[0].Status);
				Assert.Equal (AdminReflowStatus.Invalid, response.Results[1].Status);
				Assert.Equal (AdminReflowStatus.Accepted, response.Results[2].Status);
			}
			...
			private static AdminApiController CreateController (
				AdminReflowRequest request = null,
