    ...
    public class FederatedCredentialServiceFacts
    {
        public class TheAddEntraIdServicePrincipalPolicyAsyncMethod : FederatedCredentialServiceFacts
        {
            ...
            public async Task AddsPolicy()
            {
                ...
                Assert.Same(CurrentUser, result.Policy.User);
                Assert.Same(PackageOwner, result.Policy.Owner);
                ...
                Assert.Null(result.Policy.LastUpdated);
        ...
        public class TheDeletePolicyAsyncMethod : FederatedCredentialServiceFacts
        {
            ...
            public async Task DeletesCredentialAndPolicies()
            {
                ...
                // Assert
                AuthenticationService.Verify(x => x.RemoveCredential(Policies[0].User, Credential, false), Times.Once);
        ...
        public class TheGenerateApiKeyAsyncMethod : FederatedCredentialServiceFacts
        {
            ...
            public async Task RejectsCredentialWithInvalidScopes()
            {
                ...
                Assert.Null(Evaluation.Policy.LastMatched);
            }
            ...
            public async Task RejectsSaveViolatingUniqueConstraint(int sqlErrorCode)
            {
                ...
                Assert.Equal(new DateTime(2024, 10, 12, 12, 30, 0, DateTimeKind.Utc), Evaluation.Policy.LastMatched);
            }
            ...
            public async Task ReturnsCreatedApiKey()
            {
                ...
                Assert.Same(PackageOwner, Evaluation.Policy.Owner);
                Assert.Equal(new DateTime(2024, 10, 12, 12, 30, 0, DateTimeKind.Utc), Evaluation.Policy.LastMatched);

                UserService.Verify(x => x.FindByUsername(CurrentUser.Username, false), Times.Once);
                FederatedCredentialRepository.Verify(x => x.GetPoliciesForUser(CurrentUser.Key), Times.Once);
                ...
                CredentialBuilder.Verify(x => x.CreateShortLivedApiKey(TimeSpan.FromMinutes(15), Evaluation.Policy, out PlaintextApiKey), Times.Once);
                CredentialBuilder.Verify(x => x.VerifyScopes(CurrentUser, Credential.Scopes), Times.Once);
                FederatedCredentialRepository.Verify(x => x.SaveFederatedCredentialAsync(Evaluation.FederatedCredential, false), Times.Once);
        ...
        public FederatedCredentialServiceFacts()
        {
            ...
            {
                new() { Key = 3, User = CurrentUser, UserKey = CurrentUser.Key, Owner = PackageOwner, OwnerKey = PackageOwner.Key }
            };
            Evaluation = EvaluatedFederatedCredentialPolicies.MatchedPolicy(
                results: [],
                policy: Policies[0],
            ...
            FederatedCredentialRepository.Setup(x => x.GetPoliciesForUser(CurrentUser.Key)).Returns(() => Policies);
            ...
            CredentialBuilder
                .Setup(x => x.CreateShortLivedApiKey(TimeSpan.FromMinutes(15), Evaluation.Policy, out It.Ref<string>.IsAny))
