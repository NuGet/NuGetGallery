...
namespace NuGetGallery.App_Start
{
    public class ConfigurationServiceFacts
    {
        public class TheGetSiteRootMethod
        {
            ...
            [Fact]
            public async void WillGetTheConfiguredHttpSiteRoot()
            {
                var configuration = new TestableConfigurationService();
                configuration.StubConfiguredSiteRoot = "http://theSiteRoot/";

                var siteRoot = await configuration.GetSiteRoot(useHttps: false);

                Assert.Equal("http://theSiteRoot/", siteRoot);
            }

            [Fact]
            public async void WillGetTheConfiguredHttpsSiteRoot()
            {
                var configuration = new TestableConfigurationService();
                configuration.StubConfiguredSiteRoot = "http://theSiteRoot/";

                var siteRoot = await configuration.GetSiteRoot(useHttps: true);

                Assert.Equal("https://theSiteRoot/", siteRoot);
            }

            [Fact]
            public async void WillUseTheActualRootWhenTheRequestIsLocal()
            {
                var configuration = new TestableConfigurationService();
            ...
            [Fact]
            public async void WillUseHttpUponRequestWhenConfiguredSiteRootIsHttps()
            {
                var configuration = new TestableConfigurationService();
            ...
            [Fact]
            public async void WillThrowIfConfiguredSiteRootIsNotHttpOrHttps()
            {
                var configuration = new TestableConfigurationService();
                configuration.StubConfiguredSiteRoot = "ftp://theSiteRoot/";

                await Assert.ThrowsAsync<InvalidOperationException>(async () => await configuration.GetSiteRoot(useHttps: false));
            }

            [Fact]
            public async void WillCacheTheSiteRootLookup()
            {
                var configuration = new TestableConfigurationService();
                await configuration.GetSiteRoot(useHttps: false);

                await configuration.GetSiteRoot(useHttps: true);
