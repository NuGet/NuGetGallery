...
@"{
    ...
    ""catalog:commitTimeStamp"": {
      ""@type"": ""xsd:dateTime""
        ...
        internal CatalogPackageDetails(string id = null, string version = null)
        {
            var now = DateTimeOffset.UtcNow;

            Id = id ?? TestUtility.CreateRandomAlphanumericString();

            var build = (int)(now.Ticks & 0xff); // random number

            VerbatimVersion = version ?? $"1.0.{build}";
            Version = version ?? VerbatimVersion;

            IdKeyword = $"https://nuget.test/v3-catalog0/data/{now.ToString("yyyy.MM.dd.HH.mm.ss")}/{Id}.{Version}.json";
            ...
            CommitId = Guid.NewGuid().ToString("D");

            const string datetimeFormat = "yyyy-MM-ddTHH:mm:ss.FFFZ";

            CommitTimeStamp = now.ToString("yyyy-MM-ddTHH:mm:ss.FFFFFFFZ");
            Created = now.AddHours(-2).ToString(datetimeFormat);
            Description = TestUtility.CreateRandomAlphanumericString();
            LastEdited = now.AddHours(-1).ToString(datetimeFormat);
            Listed = true;
            PackageHash = CreateFakePackageHash();
            PackageHashAlgorithm = TestUtility.CreateRandomAlphanumericString();
            PackageSize = (int)(now.Ticks & 0xffffff);  // random number
            Published = Created;
            RequireLicenseAcceptance = now.Ticks % 2 == 0;
            ...
            PackageEntries = new[]
            {
                new CatalogPackageEntry(
                    ...
                    compressedLength: (int)(now.Ticks & 0xffff) + 1,
                    fullName: $"{Id}.nuspec",
                    length: (int)(now.Ticks & 0xfff) + 1,
                    name: $"{Id}.nuspec"),
                new CatalogPackageEntry(
                    idKeyword: $"{IdKeyword}#.signature.p7s",
                    typeKeyword: CatalogConstants.PackageEntry,
                    compressedLength: (int)(now.Ticks & 0xffff),
                    fullName: ".signature.p7s",
                    length: (int)(now.Ticks & 0xfff),
                    name: ".signature.p7s")
            };
